...
async function logIn () {
  try {
    const log_in = await axios.post(`${baseURL}/login`, body);
    const create_card = await createUserCard(log_in.data);
  } catch(err) {
    console.log(err)
    alert('Uh oh. Your request did not work.')
  }
...
async function regExec() {
  try {
    const reg = axios.post(`${baseURL}/register`, body)
    const create_card = await createUserCard(reg.data);
  } catch(err) {
    console.log(err)
    alert('Uh oh. Your request did not work.')
  }
