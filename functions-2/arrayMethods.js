...
// CODE HERE
const evenUmbers = (wholeArray) => wholeArray.filter((element) => element % 2 == 0);
...
// CODE HERE
const postTaxPrices = prices.map(price => parseFloat((price * 1.07).toFixed(2)));
...
// CODE HERE
const totalPopulation = (array) => array.reduce((i, j) => i + j);
...
// CODE HERE
const myStrongest = function (array) {
  for (monster of array) {
    return array.filter(monster => monster['CP'] > 200)
  }
}
...
const total = orders.map( order => {
  return parseFloat((order['price'] * (order['tax'] += 1)).toFixed(2));
})
...
function total (item, owner = 'Bob') {
  if (item['owner'] === owner) {
    bobsTotal += item['price']
  }
}