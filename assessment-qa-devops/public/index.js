...
var rollbar = new Rollbar({
  accessToken: 'a3d3f4c4e1f54801bae5b0bd318f6f9c',
...
rollbar.log('Rollbar test message');
...
const makeRobotChoiceCard = (bot) => {
    Rollbar.info("Robot choice cards");
    return `
...
const makeRobotPlayerCard = (bot) => {
    Rollbar.info("Robot player cards");
    return `
        <div class="bot-card outline">
        <img src='${bot.imgAddress}' alt='${bot.name}'/>
        <h3>${bot.name}</h3>
...
const makeRobotDisplayCard = (bot) => {
    Rollbar.info("Robot display cards");
    return `
        <div class="bot-card outline">
        <img src='${bot.imgAddress}' alt='${bot.name}'/>
        <h3>${bot.name}</h3>
...
const chooseBot = (id) => {
    if (playerDuo.length === 2) {
        Rollbar.warning("Player can only choose two bots!");
    ...
    if (playerDuo.length === 2) {
        ...
        Rollbar.info("Player has chosen cards");
    }
}
...
const putBotBack = (id) => {
    try {
        ...
        if (playerDuo.length === 0) {
            yourDuoHeader.classList.add('hide')
            Rollbar.info("Empty player cards");
        }
    } catch (err) {
...
const duel = () => {
    try{
        ...
        chooseHeader.classList.add('hide')
        renderCompDuo()
        Rollbar.debug("Duel is starting");
